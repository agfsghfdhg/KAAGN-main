...
from modeling.modeling_encoder import TextEncoder, MODEL_NAME_TO_CLASS, GraphConvolution, GCN
...
class KAAGN_Message_Passing(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, input_size, hidden_size, output_size,
        ...
        self.k = k
        self.gnn_layers = nn.ModuleList([KAAGN(args, hidden_size, n_ntype, n_etype, self.edge_encoder) for _ in range(k)])
    ...
    def mp_helper(self, _X, edge_index, edge_type, _node_type, _node_feature_extra, node_score,
                  batch=None, gnn_mask=None,last_hidden_states=None, lm_mask=None):

        _batch_size, _n_nodes = gnn_mask.size()

        for _ in range(self.k):

            _X, new_edge_index_alpha = self.gnn_layers[_](_X, edge_index, edge_type, _node_type, _node_feature_extra,
                                                          node_score, return_attention_weights=True)
            ...
            _X, edge_index, edge_type, _node_type, batch, perm, score = self.SAGPools[_](_X, lm_node_scores, edge_index,
                                                                                         edge_type, _node_type, batch,
                                                                                         None)
    ...
    def forward(self, H, A, node_type, node_score, cache_output=False,
        ...
        ## kang start
        _X, batch, new_mask = self.mp_helper(_X, edge_index, edge_type, _node_type, _node_feature_extra, node_score,
        ...
        X = _X.view(node_type.size(0), num_nodes[0], -1)
...
class QAGNN(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, sent_dim,
        ...
        self.gnn = KAAGN_Message_Passing(args, k=k, n_ntype=n_ntype, n_etype=n_etype,
                                        input_size=concept_dim, hidden_size=concept_dim, output_size=concept_dim, dropout=p_gnn)

        # 添加JOINTLK模块：联合模块和剪枝模块
        # self.jointlk = JOINT_Message_Passing(args, k=k, n_ntype=n_ntype, n_etype=n_etype,
        #                                 input_size=concept_dim, hidden_size=concept_dim, output_size=concept_dim, dropout=p_gnn)
    ...
    def forward(self, sent_vecs, concept_ids, node_type_ids, node_scores,
        ...
        # print("sent_vecs_for_pooler.size()", sent_vecs_for_pooler.size())
        # print("gnn_output.size()", gnn_output.size())
        # print("mask.size()", mask.size())

        expanded_x = torch.zeros((gnn_output.size(0), 200, 200)).cuda()
        expanded_x[:, :gnn_output.size(1), :gnn_output.size(2)] = gnn_output
        gnn_output = expanded_x
        graph_vecs, pool_attn = self.pooler(sent_vecs_for_pooler, gnn_output, mask)

        # print("graph_vecs.size()", graph_vecs.size())
        # print("pool_attn.size()", pool_attn.size())
...
class LM_QAGNN(nn.Module):
    ...
    def forward(self, *inputs, fsents, layer_id=-1, cache_output=False, detail=False):
        """
        ...
        returns: (batch_size, 1)
        """
        # inputs[0].size(0)=128, input[0].size(1)=5
        bs, nc = inputs[0].size(0), inputs[0].size(1)

        #Here, merge the batch dimension and the num_choice dimension
        edge_index_orig, edge_type_orig = inputs[-2:]   # dim:128*5*100
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type = _inputs
        ...
        # ### start 加入依存句法树 sent_vecs: 问句经过LM后得到的embedding
        # 问题节点特征
        if(len(fsents) > 1):
            question_feature = self.encoder.question_encoder(''.join([x for x in fsents]))
            dependency_matrix = self.encoder.dependency_tree(''.join([x for x in fsents]))
        else:
            question_feature = self.encoder.question_encoder(fsents[0])
            dependency_matrix = self.encoder.dependency_tree(fsents[0])
        # 依存句法树处理后的问句向量
        dependency_matrix = torch.from_numpy(dependency_matrix)
        if (question_feature.size(0) > dependency_matrix.size(0)):
            expanded_x = torch.zeros((question_feature.size(0), question_feature.size(0))).cuda()
            expanded_x[:dependency_matrix.size(0), :dependency_matrix.size(1)] = dependency_matrix
            dependency_matrix = expanded_x
        else:
            expanded_x = torch.zeros((dependency_matrix.size(0), question_feature.size(1))).cuda()
            expanded_x[:question_feature.size(0), :question_feature.size(1)] = question_feature
            question_feature = expanded_x

        # 将问句向量经过GCN处理
        gcn = GCN(input_dim=question_feature.size(1), hidden_dim=512, output_dim=question_feature.size(1))
        gcn = gcn.to(torch.device("cuda:0"))
        question_feature = question_feature.to(torch.device("cuda:0"))
        dependency_matrix = dependency_matrix.to(torch.device("cuda:0"))
        dependency_vec = gcn(question_feature, dependency_matrix)
        dependency_vec = dependency_vec.view(1024, -1)
        myline = torch.nn.Linear(dependency_vec.size(1), sent_vecs.size(0)).cuda()
        dependency_vec = myline(dependency_vec)
        dependency_vec = dependency_vec.view(sent_vecs.size(0), 1024)

        # 经过依存句法树处理的问题与LM的问题表示进行组合
        dependency_vec = dependency_vec.cuda()

        sent_vecs = sent_vecs + dependency_vec
        # ### end 加入依存句法树
        ...
        # print(type(sent_vecs))
        #
        # print(dependency_vecs.size())
        # print(type(dependency_vecs))
...
class LM_QAGNN_DataLoader(object):
    ...
    def __init__(self, args, train_statement_path, train_adj_path,
        ...
        # load_input_tensors 将数据集变为张量
        # example_ids 9714     all_label 9714
        # *data_tensors:包含4个变量 选项的4个特征张量 all_input_ids, all_input_mask, all_segment_ids, all_output_mask

        # 训练集：均为9741*5*100(9741条数据*5个选项*100分词长度)
        self.train_qids, self.train_labels, self.train_sents, *self.train_encoder_data = load_input_tensors(train_statement_path, model_type, model_name, max_seq_length)
        # 验证集：均为1221*5*100(1221条数据*5个选项*100分词长度)
        self.dev_qids, self.dev_labels, self.dev_sents, *self.dev_encoder_data = load_input_tensors(dev_statement_path, model_type, model_name, max_seq_length)

        num_choice = self.train_encoder_data[0].size(1)
        self.num_choice = num_choice
        print ('num_choice', num_choice)
        # train_decoder_data列表：4个9741*5*200张量元素  concept_ids, node_type_ids, node_scores, adj_lengths
        # train_adj_data元组：(edge_index, edge_type)
        ...
        *self.dev_decoder_data, self.dev_adj_data = load_sparse_adj_data_with_contextnode(dev_adj_path, max_node_num, num_choice, args)
        ...
        if test_statement_path is not None:
            self.test_qids, self.test_labels, self.test_sents, *self.test_encoder_data = load_input_tensors(test_statement_path, model_type, model_name, max_seq_length)
    ...
    def train(self):
        ...
        return MultiGPUSparseAdjDataBatchGenerator(self.device0, self.device1, self.batch_size, train_indexes, self.train_qids, self.train_labels, self.train_sents, tensors0=self.train_encoder_data, tensors1=self.train_decoder_data, adj_data=self.train_adj_data)

    def train_eval(self):
        return MultiGPUSparseAdjDataBatchGenerator(self.device0, self.device1, self.eval_batch_size, torch.arange(len(self.train_qids)), self.train_qids, self.train_labels, self.train_sents, tensors0=self.train_encoder_data, tensors1=self.train_decoder_data, adj_data=self.train_adj_data)

    def dev(self):
        return MultiGPUSparseAdjDataBatchGenerator(self.device0, self.device1, self.eval_batch_size, torch.arange(len(self.dev_qids)), self.dev_qids, self.dev_labels, self.dev_sents, tensors0=self.dev_encoder_data, tensors1=self.dev_decoder_data, adj_data=self.dev_adj_data)

    def test(self):
        if self.is_inhouse:
            return MultiGPUSparseAdjDataBatchGenerator(self.device0, self.device1, self.eval_batch_size, self.inhouse_test_indexes, self.train_qids, self.train_labels, self.train_sents, tensors0=self.train_encoder_data, tensors1=self.train_decoder_data, adj_data=self.train_adj_data)
        else:
            return MultiGPUSparseAdjDataBatchGenerator(self.device0, self.device1, self.eval_batch_size, torch.arange(len(self.test_qids)), self.test_qids, self.test_labels, self.test_sents, tensors0=self.test_encoder_data, tensors1=self.test_decoder_data, adj_data=self.test_adj_data)
...
class KAAGN(MessagePassing):
    ...
    def __init__(self, args, emb_dim, n_ntype, n_etype, edge_encoder, head_count=4, aggr="add"):
        super(KAAGN, self).__init__(aggr=aggr)
        self.args = args
