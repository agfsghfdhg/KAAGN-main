...
def load_cpnet(cpnet_graph_path):
    ...
    # 通过遍历 cpnet 的边，构建了一个简化的图 cpnet_simple。
    # 对于每条边，如果在 cpnet_simple 中已经存在该边，则将权重累加；否则，添加该边并设置权重
...
def concepts2adj(node_ids):
    ...
    adj = coo_matrix(adj.reshape(-1, n_node))   # 维度：二维：(n_rel*n_node， n_node)  然后转换成稀疏矩阵，稀疏矩阵存储方式只存储非零元素的位置和值，可以节省内存空间
    return adj, cids
...
TOKENIZER = RobertaTokenizer.from_pretrained('/home/xyNLP/data/kl/DRGN-main/modeling/roberta-large')
LM_MODEL = RobertaForMaskedLMwithLoss.from_pretrained('/home/xyNLP/data/kl/DRGN-main/modeling/roberta-large')
...
"""
这个函数将保存
(1)邻接矩阵(各为(R*N, N) coo稀疏矩阵)
(2)常识id（结点）
(3)qmask表示mask掉一个qcid（问题结点）
(4)amask表示mask掉一个acid（选项结点）
(5) cid2score表示常识id（结点）与question的相关分数
"""
def generate_adj_data_from_grounded_concepts__use_LM(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    ...
    """
    print(f'generating adj data for {grounded_path}...')
    # concept2id字典：concept.txt中的id：实体
    # id2concept：concept2id字典中的元素
    # relation2id字典：17种合并关系 id:关系
    # id2relation：17种关系
    global concept2id, id2concept, relation2id, id2relation, cpnet_simple, cpnet
    if any(x is None for x in [concept2id, id2concept, relation2id, id2relation]):
        load_resources(cpnet_vocab_path)
