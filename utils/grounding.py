...
def ground_qa_pair(qa_pair):
    # 声明全局变量nlp和matcher，用于存储Spacy的自然语言处理对象和匹配器对象
    global nlp, matcher
    if nlp is None or matcher is None:
        # 加载Spacy的英文模型（"en_core_web_sm"）并禁用命名实体识别（NER）、解析器（parser）和文本分类（textcat）功能
        nlp = spacy.load('en_core_web_sm', disable=['ner', 'parser', 'textcat'])
        # 添加一个句子分割器（sentencizer）到处理管道中
        nlp.add_pipe(nlp.create_pipe('sentencizer'))
        # 加载匹配器对象matcher，该匹配器对象根据预定义的模式（通过load_matcher函数加载）进行概念匹配
        matcher = load_matcher(nlp, PATTERN_PATH)

    s, a = qa_pair
    # 匹配问题和选项在concepts实体集中出现的实体
    all_concepts = ground_mentioned_concepts(nlp, matcher, s, a)
    # 选项以及包含选项的词在concepts中的实体
...
# 用于在给定的句子中执行概念匹配，返回匹配到的概念集合
def ground_mentioned_concepts(nlp, matcher, s, ans=None):
    # 将句子转换为小写，并使用Spacy的自然语言处理对象nlp对句子进行处理，得到文档对象doc
    s = s.lower()
    doc = nlp(s)

    # 使用预定义的匹配器matcher对文档进行匹配，获取所有匹配结果，存储在matches中
    matches = matcher(doc)
    ...
    if ans is not None:
        # 创建一个新的匹配器ans_matcher，用于排除答案句子中的匹配结果
...
def match_mentioned_concepts(sents, answers, num_processes):
    ...
    with Pool(num_processes) as p:
        # 将（问题描述， 答案选项）这个元组传入ground_qa_pair这个函数
...
# 用于将文本中提到的概念进行匹配和处理，并将结果保存到输出文件中
# 参数：文本语句的文件路径， CPNet词汇表的文件路径，
def ground(statement_path, cpnet_vocab_path, pattern_path, output_path, num_processes=1, debug=True):
    global PATTERN_PATH, CPNET_VOCAB
    if PATTERN_PATH is None:
        PATTERN_PATH = pattern_path
        CPNET_VOCAB = load_cpnet_vocab(cpnet_vocab_path)    # 加载CPNet的词汇表

    sents = []  # 所有的选项带入描述 9741*5=48705条数据
    answers = []    #所有的选项 9741*5=48705个选项
    with open(statement_path, 'r') as fin:
        lines = [line for line in fin]  # 读取文本语句文件，将每行文本存储在列表lines中
    ...
    for line in lines:
        if line == "":
            continue
        j = json.loads(line)    # 对于每一行文本，解析为JSON格式，提取出语句列表和答案列表
        # {'answerKey': 'B',
        ...
        for statement in j["statements"]:
            sents.append(statement["statement"])    # sent: 包含每个选项的问题描述

        for answer in j["question"]["choices"]:
            ans = answer['text']                    # ans: 问题答案
            ...
            try:
                ...
                print(ans)
            answers.append(ans)
    # 使用多进程并行地对句子和答案进行概念匹配操作，并返回匹配结果列表
    res = match_mentioned_concepts(sents, answers, num_processes)
    # 根据特定规则对数据中的问题概念和答案概念进行修剪操作，并返回修剪后的数据。修剪操作包括删除特定后缀、删除包含停用词的概念以及验证概念是否存在于CPNet词汇表中。
