...
from modeling.modeling_kaagn import *
...
print(socket.gethostname()) # 服务器名
print ("pid:", os.getpid()) # 进程号
...
def evaluate_accuracy(eval_set, model):
    ...
    model.eval()
    with torch.no_grad():
        for qids, labels, sents, *input_data in tqdm(eval_set):
            logits, _ = model(*input_data, fsents = sents)
...
def main():
    ...
    # parser.add_argument('--train_adj', default='data/{args.dataset}/graph/train.graph.adj.pk')
    parser.add_argument('--train_adj', default='data/{args.dataset}/graph/train.graph.adj.pk')
    # parser.add_argument('--dev_adj', default='data/{args.dataset}/graph/dev.graph.adj.pk')
    parser.add_argument('--dev_adj', default='data/{args.dataset}/graph/dev.graph.adj.pk')
    # parser.add_argument('--test_adj', default='data/{args.dataset}/graph/test.graph.adj.pk')
    parser.add_argument('--test_adj', default='data/{args.dataset}/graph/test.graph.adj.pk')
...
def train(args):
    ...
    # 保存参数信息、每批次的模型、日志文件到指定目录中
    config_path = os.path.join(args.save_dir, 'config.json')
    model_path = os.path.join(args.save_dir, 'model.pt')
    log_path = os.path.join(args.save_dir, 'log.csv')
    export_config(args, config_path)    # 将配置参数信息以json格式写入指定文件
    ...
    #   Load data                                                                                     #
    ###################################################################################################
    cp_emb = [np.load(path) for path in args.ent_emb_paths] #加载实体嵌入文件
    cp_emb = torch.tensor(np.concatenate(cp_emb, 1), dtype=torch.float) #将实体嵌入的数组转换为张量

    #concept中的实体个数为799273，维度为1024
    ...
    if True:
        ...
        # 加载数据集
        dataset = LM_QAGNN_DataLoader(args, args.train_statements, args.train_adj,# 稀疏邻接矩阵
    ...
    #  学习率调度器的类型
    if args.lr_schedule == 'fixed': # 固定学习率
        try:
            scheduler = ConstantLRSchedule(optimizer)
        except:
            scheduler = get_constant_schedule(optimizer)
    elif args.lr_schedule == 'warmup_constant': # 使用较低的学习率预热，再固定学习率
        try:
            scheduler = WarmupConstantSchedule(optimizer, warmup_steps=args.warmup_steps)
        except:
            scheduler = get_constant_schedule_with_warmup(optimizer, num_warmup_steps=args.warmup_steps)
    elif args.lr_schedule == 'warmup_linear':   # 预热阶段的学习率逐渐增加
    ...
    if True:
    # try:
        for epoch_id in range(args.n_epochs):   #args.n_epochs=100
            if epoch_id == args.unfreeze_epoch: #args.unfreeze_epoch=4
                unfreeze_net(model.encoder) #第4个epoch时解除固定梯度下降
            if epoch_id == args.refreeze_epoch: #args.refreeze_epoch=100000
                freeze_net(model.encoder) # 第100000个epoch时固定梯度下降
            model.train()
            for qids, labels, sents, *input_data in dataset.train():   # dataset.train()中以batchsize为步长，第一个元素中有batchsize条数据
                # qids=train_qids labels=train_labels
                # *input_data=[device0, device1, batch_size, train_indexes, train_encoder_data, train_decoder_data, train_adj_data]
                optimizer.zero_grad()
                bs = labels.size(0)
                # 循环遍历从 0 到 128（不包括 128）的整数，步长为 1
                for a in range(0, bs, args.mini_batch_size):    # args.mini_batch_size=1
                    b = min(a + args.mini_batch_size, bs)   #b=为小于128的数，初始为1，并且每次循环都加1
                    logits, _ = model(*[x[a:b] for x in input_data], fsents = [sents[a]], layer_id=args.encoder_layer)
            ...
            if not save_test_preds:
                ...
                #         for qids, labels, sents, *input_data in tqdm(eval_set):
                #             count += 1
                #             logits, _, concept_ids, node_type_ids, edge_index, edge_type = model(*input_data, fsents = sents, detail=True)
